...
#k_sig1 =  np.array([0,4])
#k_sig2 =  np.array([1,5,9,13])
#k_sig3 =  np.array([2,6,10,14])
#k_sig4 =  np.array([3,9,11,15])
...
for n,k in enumerate(comb_array_sig[:,:]):
    ...
    print(kg.shape)
    ct = int(0.9*(kg.shape[0])) # 0.7
    print(kg[ct])
    zeros[(features[:,:,k[3]] >  -0.005) & (features[:,:,k[2]] >  -0.005)  & (features[:,:,k[0]] < 1.0)] = 1 # 0.9
    zeros[(features[:,:,k[3]] <  -0.003) & (features[:,:,k[2]] >  -0.005)  & (features[:,:,k[0]] < 5.0)] = 2 #  & (features[:,:,0] > 4.5) ] = 2
    #zeros[(features[:,:,k[3]] >  -0.005) & (features[:,:,k[2]] <  -0.005)  & (features[:,:,k[0]] < 3.5)] = 2 
    ...
    H1_4d, edges1_4d = np.histogramdd(sele1, bins = (b1, b2, b3, b4), density = False)
    H1_4d = H1_4d.reshape(bq) # /np.sum(H1_4d)
    en1_4d = entropy(H1_4d)
    #print("en_ld",en1_4d)

    H2_4d, edges2_4d = np.histogramdd(sele2, bins = (b1, b2, b3, b4), density = False)

    #print("edges", edges2_4d)

    H2_4d = H2_4d.reshape(bq) # /np.sum(H2_4d)
    en2_4d = entropy(H2_4d)
    #print("en_id",en2_4d)

    H3_4d, edges3_4d = np.histogramdd(sele3, bins = (b1, b2, b3, b4), density = False)
    H3_4d = H3_4d.reshape(bq) # /np.sum(H3_4d)
    en3_4d = entropy(H3_4d)
  
    Hall_4d, edgesall_4d = np.histogramdd(seles, bins = (b1, b2, b3, b4), density = False)
    Hall_4d = Hall_4d.reshape(bq) # /np.sum(Hall_4d)
    enall_4d = entropy(Hall_4d)
   
    entrop_arr = np.array([k[0],k[1],k[2],k[3],enall_4d,en1_4d,en2_4d,en3_4d])
    tr_entrops.append(entrop_arr)
    ...
    coordinates_highs = remove_small(conn_labels_sigma05_highs,0)[0]
    num_regions_highs = remove_small(conn_labels_sigma05_highs,0)[1]
    coordinates_netwo = remove_small(conn_labels_sigma05_netwo,0)[5]
    num_regions_netwo = remove_small(conn_labels_sigma05_netwo,0)[1]
    coordinates_empty = remove_small(conn_labels_sigma05_empty,0)[5]
    num_regions_empty = remove_small(conn_labels_sigma05_empty,0)[1]
    area_empty = remove_small(conn_labels_sigma05_empty,0)[4]
    area_netwo = remove_small(conn_labels_sigma05_netwo,0)[4]
    ...
    coordinates_netwo_emp = remove_small(conn_labels_sigma05_netwo_emp,0)[0]
    num_regions_netwo_emp = remove_small(conn_labels_sigma05_netwo_emp,0)[1]
    
    
    coordinates_highs_all = remove_small(conn_labels_sigma05_highs_all,0)[0]
    num_regions_highs_all = remove_small(conn_labels_sigma05_highs_all,0)[1]
    ...
    coordinates_all = remove_small(labels_all,0)[0]
    ...
    num_regions_highs_merged     = remove_small(highs_merged,0)[1]  # 25 75 25  50  100
    areas_highs_merged           = remove_small(highs_merged,0)[4]  # 25 75 25  50  100
    coordinates_highs_merged     = remove_small(highs_merged,0)[5]
    ...
    ce_4d, edges_ce_4d = np.histogramdd(ce, bins = edges1_4d, density = False)
    ce_4d = ce_4d.reshape(bq)  # /np.sum(ce_4d)
    ce_en_4d = entropy(ce_4d)
    #print("ce_en_ld",ce_en_4d)

    #cn_4d, edges_cn_4d = np.histogramdd(cn, bins = (b1, b2, b3, b4), density = False)
    cn_4d, edges_cn_4d = np.histogramdd(cn, bins = edges2_4d , density = False)
    cn_4d = cn_4d.reshape(bq)  # /np.sum(cn_4d)
    cn_en_4d = entropy(cn_4d)
    #print("edges", edges_cn_4d)

    #print("cn_en_id",cn_en_4d)

    #ch_4d, edges_ch_4d = np.histogramdd(ch, bins = (b1, b2, b3, b4), density = False)
    ch_4d, edges_ch_4d = np.histogramdd(ch, bins = edges3_4d, density = False)
    ch_4d = ch_4d.reshape(bq)   # /np.sum(ch_4d)
    ch_en_4d = entropy(ch_4d)
    #print("ch_en_hd",ch_en_4d)

    #chs_4d, edges_chs_4d = np.histogramdd(chs, bins = (b1, b2, b3, b4), density = False)
    chs_4d, edges_chs_4d = np.histogramdd(chs, bins = edgesall_4d, density = False)
    chs_4d = chs_4d.reshape(bq)  # /np.sum(chs_4d)
    chs_en_4d = entropy(chs_4d)
    #print("chs_en_all",chs_en_4d)

    entrop_pr_arr = np.array([k[0],k[1],k[2],k[3],chs_en_4d,ce_en_4d,cn_en_4d,ch_en_4d])
    entrops_pred.append(entrop_pr_arr)
    ...
    ##np.savetxt('inf_probab.txt' ,p , fmt = "%s"  )
    ...
    #print("edges", edges2_4d)
    bin_end = len(cn_4d) + 1
    bins_n = np.arange(1,bin_end,1)


    entro_rel_em = distance.jensenshannon(ce_4d,H1_4d)
    #print("ens_em",entro_rel_em)

    entro_rel_ne = distance.jensenshannon(cn_4d,H2_4d)
    #print("ens_ne",entro_rel_ne)

    entro_rel_hi = distance.jensenshannon(ch_4d,H3_4d)
    #print("ens_hi",entro_rel_hi)

    entro_rel_all = distance.jensenshannon(chs_4d,Hall_4d)
 
 
    print("jensenshannon_all",k[0],k[1],k[2],k[3], entro_rel_all,entro_rel_em,entro_rel_ne,entro_rel_hi)
    ...
    rel_entro = np.array([k[0],k[1],k[2],k[3],entro_rel_all,entro_rel_em,entro_rel_ne,entro_rel_hi,mini1,mini2,mini3,ce.shape[0],cn.shape[0],ch.shape[0],imp[0],imp[1],imp[2],imp[3],IG,IE1,IE2,GE1,GE2,E1E2])

    rel_entros.append(rel_entro)
    #print(rel_entros)


feats=['I','G','E1','E2','wass_ovr',"wass_LD","wass_ID","wass_HD","dim TD LD","dim TD ID","dim TD HD","dim LD_pr","dim ID_pr","dim HD_pr","I_imp","G_imp","E1_imp","E2_imp","IG_cr","IE1_cr","IE2_cr","GE1_cr","GE2_cr","E1E2_cr"] 

df_entro = pd.DataFrame(data = rel_entros,columns = feats) 
df_entro.iloc[:,0:4] = df_entro.iloc[:,0:4].astype(int)
df_entro.iloc[:,:].to_csv(r"inf_jensenshannon_e1e2_4feat_bminus01.csv")

feats2=['I','G','E1','E2','H_ovr',"H_LD","H_ID","H_HD"] 

tr_entro = pd.DataFrame(data = tr_entrops,columns = feats2) 
tr_entro.iloc[:,0:4] = tr_entro.iloc[:,0:4].astype(int)
tr_entro.iloc[:,:].to_csv(r"inf_tr_entropies_e1e2_4feat_bminus01.csv")

pr_entro = pd.DataFrame(data = entrops_pred,columns = feats2) 
pr_entro.iloc[:,0:4] = pr_entro.iloc[:,0:4].astype(int)
pr_entro.iloc[:,:].to_csv(r"inf_pr_entropies_e1e2_4feat_bminus01.csv")
...
minx = np.min(sup_x100ns) #  + 0.5
miny = np.min(sup_y100ns) #  + 0.5
maxx = np.max(sup_x100ns) #  - 0.5
maxy = np.max(sup_y100ns) #  - 0.5
...
for n,k in enumerate(comb_array_sig[:,:]):
    ...
    prob =  classif.predict_proba(features[:,:,[k[0],k[1],k[2],k[3]]].reshape(new_sh,features[:,:,[k[0],k[1],k[2],k[3]]].shape[2]))
    df = pd.DataFrame(classif.predict_proba(features[:,:,[k[0],k[1],k[2],k[3]]].reshape(new_sh,features[:,:,[k[0],k[1],k[2],k[3]]].shape[2])))
    ##np.savetxt('sup_probab.txt' ,p , fmt = "%s"  )
...
df_entro = pd.DataFrame(data = rel_entros,columns = feats) 
df_entro.iloc[:,0:4] = df_entro.iloc[:,0:4].astype(int)
df_entro.iloc[:,:].to_csv(r"sup_jensenshannon_e1e2_4feat_bminus01.csv")

feats2=['I','G','E1','E2','H_ovr',"H_LD","H_ID","H_HD"] 

tr_entro = pd.DataFrame(data = tr_entrops,columns = feats2) 
tr_entro.iloc[:,0:4] = tr_entro.iloc[:,0:4].astype(int)
tr_entro.iloc[:,:].to_csv(r"sup_tr_entropies_e1e2_4feat_bminus01.csv")

pr_entro = pd.DataFrame(data = entrops_pred,columns = feats2) 
pr_entro.iloc[:,0:4] = pr_entro.iloc[:,0:4].astype(int)
pr_entro.iloc[:,:].to_csv(r"sup_pr_entropies_e1e2_4feat_bminus01.csv")
